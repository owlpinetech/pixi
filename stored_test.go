...
func TestStoredSampleFieldConcurrent(t *testing.T) {
	...
	cache := NewStoredLayer(header, layer)
	...
	for randInd := range testCoords {
		wg.Add(1)
		go testSampleAtSameAsRaw(t, &wg, rdBuffer, cache, testCoords[randInd], testExpect[randInd])
	}
	wg.Wait()
}

func testSampleAtSameAsRaw(t *testing.T, wg *sync.WaitGroup, backing io.ReadSeeker, layer *StoredLayer, coord SampleCoordinate, expect any) {
	defer wg.Done()
	at, err := layer.FieldAt(backing, coord, 1)
	if err != nil {
		t.Error(err)
	} else if at != expect {
...
func TestStoredSetSampleAt(t *testing.T) {
	...
	stored := NewStoredLayer(header, layer)

	sample0, err := stored.SampleAt(wrtBuf, SampleCoordinate{250, 250})
	if err != nil {
		t.Fatal(err)
	}
	...
	err = stored.SetSampleAt(wrtBuf, SampleCoordinate{250, 250}, []any{uint16(42), uint32(4242)})
	if err != nil {
		t.Fatal(err)
	}

	sample1, err := stored.SampleAt(wrtBuf, SampleCoordinate{250, 250})
	if err != nil {
		t.Fatal(err)
	}
